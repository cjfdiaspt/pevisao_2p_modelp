...
calcular_evolucao_acerto(st.session_state.df_base)
    ...
    if a15:
        st.markdown("- ✅ Reação esperada: Aposta recomendada **OVER 4.5 na 2ª parte** (alerta 15 ativo)")
        st.markdown("- ⛔ Nenhuma reação esperada (alerta 15 não ativo)")

    if a16:
        st.markdown("- ✅ Correção ofensiva provável: Aposta recomendada **OVER 4.5 na 1ª parte** (alerta 16 ativo)")
        st.markdown("- ⛔ Nenhuma indicação de correção ofensiva (alerta 16 não ativo)")


    st.markdown("**🧠 Análise por Alertas (1ª parte):**")
    if a4:
        st.markdown("- ⚠️ Jogo truncado: Aposta sugerida **UNDER 4.5 na 2ª parte** (alerta 4 ativo)")
    if a14:
        st.markdown("- 💥 Padrão explosivo: Aposta sugerida **OVER 4.5 na 2ª parte** (alerta 14 ativo)")
    if a17:
        st.markdown("- 📈 Tendência ofensiva: Potencial **OVER 4.5 na 2ª parte** (alerta 17 ativo)")
    if not any([a4, a14, a17]):
        st.markdown("- ℹ️ Nenhum alerta ativado com confiança suficiente na 1ª parte.")
